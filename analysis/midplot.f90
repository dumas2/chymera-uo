! use simple bilinear interpolation to make midplane maps.
...
!
 ...
 character :: filein*72, fileout*72,filenum*6,dum(13)*72
  ...
  if ( len(trim(dum(1))) == 0 .or. &
    ...
    print *, " "
    print *, " "
    print *, "midplot v 1. 11.10.2007. Nora Bolig"
    print *, " "
    print *, " "
    print *, "MIDPLOT USAGE: sigplot {stepbegin} {stepend} {stepskip}"
    print *, "                       {jmax} {kmax} {lmax} {torp} {dz}"
    print *, "                       {cell-to-size unit conversion  }"
    print *, "                       {min background}"
    print *, "                       {code to cgs conversion}"
  ...
  print *, " SIGPLOT OUT: conversion -> ", sconv
 ...
 do L = 0, LMAX-1
  K=2!do K = 0, KMAX-1
    do J = 0, JMAX-1
      ...
      array2d(J,L) = array(J,K,L) ! just midplane
    enddo
  !enddo
 enddo
