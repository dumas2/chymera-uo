...
!
 ...
 integer :: II, JJ, IRR, IAN, IAN2, I, L, J, K, IOerr

 real(KIND=8), allocatable, dimension(:,:,:) :: array
 real(KIND=8), allocatable, dimension(:,:) :: array2d
 real(KIND=8), allocatable, dimension(:) :: array1d
 ...
 character :: filein*72, fileout*72,filenum*6,dum(13)*72
  ...
  if ( len(trim(dum(1))) == 0 .or. &
    ...
    print *, " "
    print *, " "
    print *, "SIGPLOT USAGE:  overden{stepbegin} {stepend} {stepskip}"
    print *, "                       {jmax} {kmax} {lmax} {torp} {dz}"
    print *, "                       {cell-to-size unit conversion  }"
    print *, "                       {min tk                        }"
    print *, "                       {code to kelvin                }"
    print *, "                       {rhofilein prefix} {fileout prefix}" 
    print *, " "
    print *, " "
 ...
 do while ( I <= IEND )
    ...
    print "(a)", trim(filein)
    OPEN(UNIT=12, FILE=trim(filein),FORM='UNFORMATTED', STATUS="OLD", IOSTAT=IOerr)

    read(12, IOSTAT=IOerr)array
    read(12, IOSTAT=IOerr)time
    close(12)

    if(IOerr/=0) then
     print *, IOerr, "ERROR SKIPPING"
     I=I+ISKIP
     cycle
    endif

    fileout=trim(dum(13))//filenum//' ' 
    print "(a)", trim(fileout)
...
!$OMP DO SCHEDULE(STATIC)
 do L = 0, LMAX-1
  do K = 0, KMAX-1
    do J = 0, JMAX-1
      array2d(J,L) = array(J,K,L) + array2d(J,L) ! take into account one side
    enddo
  enddo
 enddo
!$OMP ENDDO
!$OMP MASTER
 do J=0,JMAX-1
   array1d(J)=array1d(J)+array2d(J,L)/dble(LMAX)
 enddo
!$OMP END MASTER
!$OMP BARRIER
!$OMP DO SCHEDULE(STATIC)
 do L = 0, LMAX-1
    do J = 0, JMAX-1
      array2d(J,L) = array2d(J,L)/array1d(J) ! take into account one side
    enddo
 enddo
!$OMP ENDDO
!$OPM END PARALLEL
...
!   if (IRR >= JMAX-1) then
!     write(13,"(3(1pe15.8,1X))") ir*rconv, jr*rconv, bg  + log10(sconv)! SWITCH: set background density
!   else
...
!   endif
   if (IRR >= JMAX-1) then
     write(14,"(3(1pe15.8,1X))") ir*rconv, jr*rconv, bg  + (sconv)! SWITCH: set background density
   else
     y1 = (array2d(IRR,IAN))
     y2 = (array2d(IRR,IAN2))
 ...
 deallocate(array,array2d,array1d)
