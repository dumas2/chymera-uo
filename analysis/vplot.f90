...
!
 ...
 dphi = 2.d0*pi/dble(LMAX)
 ir = -JMAX*1.d0-0.5d0
 do II = 0, XMAX-1,IvSKIP
  ir = ir + 1.d0
  jr = -JMAX*1.d0-0.5d0
  do JJ = 0, YMAX-1,IvSKIP
   jr = jr+1.d0
   rr = sqrt(jr**2+ir**2)
   if (ir > 0.d0)then
     angle = atan( (jr)/(ir))
   ...
   if (IRR >= JMAX-1) then
     write(13,"(4(1pe15.8,1X))") ir*rconv, jr*rconv, bg*(sconv),bg*sconv! SWITCH: set background density
   else
     y1 = (vx(IRR,IAN))
     y2 = (vx(IRR,IAN2))
     y3 = (vx(IRR+1,IAN2))
     ...
     t = (angle - IAN*1.d0)
     u = (rr - IRR*1.d0)
     vxs= ((1.-t)*(1.-u)*y1+t*(1.-u)*y2+t*u*y3+(1.-t)*u*y4)*sconv
     y1 = (vy(IRR,IAN))
     y2 = (vy(IRR,IAN2))
     y3 = (vy(IRR+1,IAN2))
     y4 = (vy(IRR+1,IAN))
     vys= ((1.-t)*(1.-u)*y1+t*(1.-u)*y2+t*u*y3+(1.-t)*u*y4)*sconv
     write(13,"(4(1pe15.8,1X))") ir*rconv, jr*rconv,vxs,vys
   endif
  enddo
  write(13,"(a)")
 enddo
