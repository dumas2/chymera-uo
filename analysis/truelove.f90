...
!
 ...
 do L = 0, LMAX-1
  do K = 0, KMAX-1
    do J = 0, JMAX-1
      sig(J,L) = sig(J,L)+ rho(J,K,L)*dz*2.d0
             ! take into account both sides of disk
      omega(J,L) = omega(J,L)+a(J,K,L)/( (dble(J)+.5d0)*dr )**2 *dz*2.
      sound(J,L) = sound(J,L)+rho(J,K,L)*sqrt(gamma1(J,K,L)*temp(J,K,L)/(muc)*rgas) &
               * dz*2.d0/(vconv)
    enddo
  enddo
 enddo
